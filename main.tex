...
\newcommand{\eamonn}[2][inline]{\todo[#1]{\textbf{eamonn:} #2}\xspace}
...
Using the adders of~\cite{cuccaro2004new,Takahashi:2008:FQC} we build reversible circuits that take as input bitstring representatives of two lattice vectors, take a bitwise \texttt{CNOT},\malb[]{Why not call it XOR?}\eamonn[]{Just to get into the quantum computational language as early as possible.} determine the Hamming weight of the outcome and further determine whether this Hamming weight is smaller than some bound.
...
An example with $d = 4, k = 1$, using the adder of~\cite{cuccaro2004new} is given in Figure~\ref{fig:naieve_popcnt}.
\malb[]{In subfigure (c), is that a three-way XOR?}
\eamonn[]{No, it's a non exclusive OR as given above, it just needs an ancillary because it isn't reversible otherwise.}
...
\malb[]{What does the \({t = \lceil\log_{2}d\rceil}\) notation mean?}
\eamonn[]{I'm introducing a dummy variable for the Euler--Maclaurin method, then evaluating it at this value.} % chktex 8
\begin{align*}
