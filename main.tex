...
\malb[]{How should I read each of the lines involving \(\oplus\)? What is XORed with what?}
\eamonn[]{Oh sorry! I misunderstood the question. The first in the OR is a Toffoli gate, the wire with the $\oplus$ is flipped iff both others are 1. The second and third gates in the OR are CNOT gates, the wires with $\oplus$ flip iff the respective wires with the thick dot are 1.}
...
Finally the depth of the circuit is calculated as  % chktex 8
\begin{align*}
...
\begin{figure}
    ...
    \begin{tabular}{lrrr}
        ...
        \cite{Takahashi:2008:FQC}   & $O(d)$    & $O\left(\log_{2}d\left(\log(\log_{2}d)\right)\right)$ & $8d + O(\log_{2}d)$\\ % chktex 2
        \bottomrule
    \end{tabular}
